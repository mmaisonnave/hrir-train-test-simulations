...
#     unlabeled = DatasetDP.get_DP_unlabeled_collection()
    ...
    results_1st_round['SCAL type']=['smart scal']
    ...
    scal_model = SCALDP(session_name='two round scal(b)',
    ...
    results_2nd_round['SCAL type']=['smart scal']
...
#     final_unlabeled = [item for item in DatasetDP.get_DP_unlabeled_collection() if not item.id_ in labeled_ids]
...
#     while get_effort(Ni, ni)<total_effort:
#         ni+=1
...
#     assert len(unlabeled)==len(DatasetDP.get_DP_unlabeled_collection())-len(labeled)
...
#     results_single_round['FN (test)']=[fn]
    ...
    for key in results_1st_round:
        results[key]=results_1st_round[key]+results_2nd_round[key]

    results['Total effort']=[results['Effort'][0],results['Effort'][0]+results['Effort'][1]]

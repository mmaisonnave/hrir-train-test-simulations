...
from lib.dataset import  DataItemDP
from lib.models import LogisticRegressionDP, SVMDP
import datetime
import logging
import pytz
import os
...
class SCALDP(object):
    ...
    def __init__(self,
        ...
        N1=min(int(full_N/2), len([item for item in unlabeled_collection if  DataItemDP.RELEVANT_LABEL==self.oracle[item.id_]]))
        N2=int(full_N-N1)
        # DataItemDP.RELEVANT_LABEL==self.oracle[item.id_]
        if smart_start:
            relevant_items=[item for item in unlabeled_collection if  DataItemDP.RELEVANT_LABEL==self.oracle[item.id_]]
            irrelevant_items=[item for item in unlabeled_collection if  DataItemDP.IRRELEVANT_LABEL==self.oracle[item.id_]]
        ...
        assert os.path.exists(os.path.join(SCALDP.repository_path,'log'))
        logging.basicConfig(filename=os.path.join(SCALDP.repository_path,'log', 'scal_ranking_function_simulation.log'), 
...
#                 count+=1
        ...
        # METRICS ON TEST #
        # --------------- #
        ytrue = np.array([True if self.oracle[item.id_]==DataItemDP.RELEVANT_LABEL else False  for item in final_unlabeled_collection])
        assert np.sum(ytrue)==len([item for item in final_unlabeled_collection if  self.oracle[item.id_]==DataItemDP.RELEVANT_LABEL])
#         print(f'size of ytrue={len(ytrue)} (relevant={len(ytrue[ytrue==1])})')
        ...
        assert tp+fn == len([item for item in final_unlabeled_collection if  self.oracle[item.id_]==DataItemDP.RELEVANT_LABEL])
...
#                       'Representation': ['TF-IDF'],
        ...
        ytrue = np.array([True if self.oracle[item.id_]==DataItemDP.RELEVANT_LABEL else False  for item in self.labeled_collection])

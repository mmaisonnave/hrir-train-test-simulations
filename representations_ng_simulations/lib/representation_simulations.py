...
import pickle
import sys
# repository_path=open('/home/ec2-user/SageMaker/mariano/repositories/train-test-split/smart-phase-scal-ng-train-test/config/repository_path.txt','r').read()
...
#              'uncertainty', 'min_distance', 'uncertainty_with_avg_diversity', 'uncertainty_with_min_diversity']
    ...
    parser.add_argument('--category', dest='category', type=str, help='Category', choices=open(os.path.join(repository_path, 'files', 'categories.txt'), 'r').read().splitlines(), required=True)
